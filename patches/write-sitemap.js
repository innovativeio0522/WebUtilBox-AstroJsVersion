...
async function writeSitemap({
  ...
  // If we have custom sitemaps OR more than one page, use index
  // Otherwise use a single sitemap file
  if (customSitemaps.length > 0 || sourceData.length > limit) {
    ...
    // Single sitemap file
    // We need to use the same approach as SitemapAndIndexStream
    ...
    const writePath = resolve(destinationDir, `./${filenameBase}.xml`);
    const src = Readable.from(sourceData);
    if (astroConfig.trailingSlash === "never" || astroConfig.build.format === "file") {
      const host = hostname.endsWith("/") ? hostname.slice(0, -1) : hostname;
      const searchStr = `<loc>${host}/</loc>`;
      const replaceStr = `<loc>${host}</loc>`;
      const replaceStream = replace(searchStr, replaceStr);
      return promisify(pipeline)(src, sitemapStream, replaceStream, createWriteStream(writePath));
    } else {
      return promisify(pipeline)(src, sitemapStream, createWriteStream(writePath));
    }
  }
}
export {
