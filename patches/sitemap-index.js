    ...
    hooks: {
      ...
      "astro:build:done": async ({ dir, pages, logger }) => {
        try {
          ...
          pageUrls = Array.from(/* @__PURE__ */ new Set([...pageUrls, ...customPages ?? []]));
          if (filter) {
...
${err.toString()}`);
          ...
          const finalFile = (customSitemaps.length > 0 || urlData.length > entryLimit) ? outFile : `${filenameBase}.xml`;
          logger.info(`\`${finalFile}\` created at \`${path.relative(process.cwd(), destDir)}\``);
        } catch (err) {
